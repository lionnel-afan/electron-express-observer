    ...
    if (!timeblock) {
        ...
        setTimeout(function () {
            ...
            child = require('child_process')
                .spawn(electron, [options.app.src],
                { stdio: 'inherit' });
            timeblock = false;
        }, 3 * 1000);
    }
    ...
    return gulp.src(path.join(options.app.src, '/index.html'))
        ...
        inject(
            gulp.src(
                ...
                //path.join(options.app.src, '/styles/sass/_*.scss')
            ),
...
gulp.task('inject-own', function () {
    ...
    // var sources = gulp.src('./bower_components/**/*.css')//, { read: false, cwd: __dirname + '/public' })
...
var buildStyles = function () {
    ...
    return gulp.src('src/styles/sass/z_main.scss')
        .pipe(wiredep())
...
gulp.task('eslint', function () {
    ...
    // console.log("ES lint path :", eslintPath)

    return gulp.src(eslintPath)
        .pipe(eslint({
            globals: ["angular", "jQuery", "require"],
...
gulp.task('inject-scripts', gulp.series('eslint', function () {

    var injectScripts = gulp.src([
        path.join(options.app.src, '/app/**/*.module.js'),
        path.join(options.app.src, '/assets/**/*.js'),
        path.join(options.app.src, '/app/**/*.js'),
        path.join('!' + options.app.src, '/app/**/*.spec.js'),
        path.join('!' + options.app.src, '/app/**/*.mock.js'),
    ])
        .on('error', conf.errorHandler('AngularFilesort'));
    ...
    return gulp.src(path.join(options.app.src, '/index.html'))
        .pipe(inject(injectScripts, { addRootSlash: false, addPrefix: '..' }))
        .pipe(gulp.dest(path.dirname(path.join(options.app.src, '/index.html'))));
}));
...
gulp.task('start-inject',
    gulp.parallel('inject-scripts',
        //'inject-bower',
        // 'sass',
        //'styles',
        //'inject-styles'
    ));// gulpsync.async([['inject-scripts', 'inject-bower'], ["sass", 'inject-styles']]));


gulp.task('assets', gulp.parallel('clean', 'imagemin', 'copy-fonts', 'bower-fonts', 'main-copy', 'node-modules-copy', 'templatecache'));


gulp.task('dist', gulp.series('start-inject', 'assets', function () {
...
---------------------------------------------------*/
gulp.task('electrify-win', gulp.series('dist', function () {
...
gulp.task('electrify-linux', gulp.series('dist', function () {
    var pkg = require(options.app.src + '/package.json');
    (require('del')).sync([
        './' + pkg.name + '-linux-ia32',
        './' + pkg.name + '-linux-x64',
...
gulp.task('electrify-mas', gulp.series('dist', function () {
    var pkg = require(options.app.src + '/package.json');
    (require('del')).sync([
        './' + pkg.name + '-mas-ia32',
        './' + pkg.name + '-mas-x64',
...
gulp.task('watch', function (done) {
    ...
    gulp.watch(path.join(options.app.src, options.css.root, options.css.src), gulp.parallel('inject-styles', restartElectron));
    gulp.watch(path.join(options.app.src, options.js.root, options.js.src), gulp.parallel('inject-scripts', restartElectron));
    gulp.watch([options.app.src + '/**/*.html', '!' + options.app.src + '/index.html'], gulp.parallel(restartElectron));
    gulp.watch(path.join(options.app.src, '/index.html'), restartElectron);
    gulp.watch(path.join('.', '/bower.json'), gulp.parallel('inject-bower', restartElectron));
});



gulp.task('run', gulp.parallel('start-inject', function () {
    restartElectron();
    gulp.parallel('watch');
    enableWatchers();
}));
...
function enableWatchers() {
    ...
    // If index .html file chages
    // var indexWatcher = gulp.watch(path.join(options.app.src, '/index.html'),
    //     gulp.parallel());

    // indexWatcher.on('change', function (path, stats) {
    //     console.log('File ' + path + ' was changed');
    //     console.log('Restarting Electron ');
    //     restartElectron();
    // });

    // if any of my JS files changes.
    var jsWatcher = gulp.watch(path.join(options.app.src, options.js.src),
        // gulp.series('inject-scripts'));
        gulp.series('start-inject'));

    jsWatcher.on('change', function (path, stats) {
        console.log('File ' + path + ' was changed');
        console.log('Restarting Electron ');
        // gulp.parallel('start-inject',  restartElectron);
        gulp.parallel('start-inject');
        restartElectron();
    });

    //if any of my sass files changes : 
    var sassWatcher = gulp.watch(path.join(options.app.src, options.css.root, options.css.sass),
        // gulp.series('sass-styles', 'inject-styles'));
        gulp.series('start-inject'));
    ...
    ///if any of my HTML files changes.

    var htmlWatcher = gulp.watch([options.app.src + '/**/*.html', '!' + options.app.src + '/index.html'],
        gulp.parallel());
